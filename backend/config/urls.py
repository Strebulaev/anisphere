...
def proxy_image(request, encoded_url):
    ...
    try:
        ...
        proxy_url = (
            f"https://images.weserv.nl/?url={encoded_original}&w=500&q=80&output=webp"
        )
        ...
        if response.status_code != 200:
            ...
            logger.warning(
                f"[PROXY] weserv failed, trying direct: {response.status_code}"
            )
            response = requests.get(
                original_url, headers=headers, timeout=15, stream=True
            )
            if response.status_code != 200:
                return HttpResponse(
...
urlpatterns = [
    ...
    re_path(
        r"^shop-verification-og7k9w1Zma\.txt$",
        lambda r: HttpResponse(
            "shop-verification-og7k9w1Zma", content_type="text/plain"
        ),
    ),
