# anime/management/commands/mass_import.py
import time
import random
...
class Command(BaseCommand):
    help = 'Массовый импорт аниме из Shikimori'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            choices=['smart', 'ultra'],
            default='smart',
            help='Режим импорта: smart (умный) или ultra (ультра-быстрый)'
        )
        parser.add_argument(
            '--workers',
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help='Ограничение количества аниме для импорта'
        )
    ...
    def handle(self, *args, **options):
        ...
        if options['mode'] == 'ultra':
            if options['limit']:
                self.stdout.write(f"Запуск ультра-быстрого импорта с ограничением {options['limit']} аниме")
                importer.import_ultra_fast(limit=options['limit'])
            else:
                self.stdout.write("Запуск ультра-быстрого импорта всех аниме")
                importer.import_ultra_fast()
        else:
            self.stdout.write("Запуск умного импорта 100000+ аниме")
...
class MassAnimeImporter:
    ...
    def import_ultra_fast(self, limit=None):
        """Ультра-быстрый импорт всех аниме подряд"""
        print("=" * 80)
        print("🚀 ULTRA FAST MODE: ИМПОРТ АНИМЕ С SHIKIMORI")
        print("=" * 80)
        ...
        if limit:
            total_ids = limit
            print(f"🎯 Цель: {total_ids} аниме (ограничение)")
        else:
            total_ids = 500000
        ...
        for start in range(1, total_ids + 1, batch_size):
            ...
            if limit and imported >= limit:
                print(f"✅ Достигнут лимит в {limit} аниме")
                break
    ...
    def _strategy_popular_ids(self):
        ...
        for start, end, target in popular_ranges:
            ...
            ids = random.sample(range(start, end + 1), min(ids_needed, end - start + 1))
