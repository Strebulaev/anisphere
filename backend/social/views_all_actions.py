...
def unpin_post(request, post_id):
    ...
    try:
        ...
        if not post.is_pinned:
            return Response(
                {"success": True, "message": "Пост уже откреплён", "is_pinned": False}
            )
        ...
        return Response(
            {"success": True, "message": "Пост откреплён", "is_pinned": False}
        )
    except Exception as e:
        logger.error(f"Error unpinning post {post_id}: {e}", exc_info=True)
        return Response(
            {"error": "Ошибка при откреплении поста", "detail": str(e)}, status=500
        )
...
def toggle_bookmark_view(request):
    ...
    if bookmark:
        bookmark.delete()
        return Response(
            {"success": True, "bookmarked": False, "message": "Удалено из закладок"}
        )
    else:
        Bookmark.objects.create(user=request.user, post=post)
        return Response(
            {"success": True, "bookmarked": True, "message": "Добавлено в закладки"}
        )
    ...
    try:
        ...
        if action == "unrepost":
            ...
            deleted = Repost.objects.filter(
                original_post=original_post, user=request.user
            ).delete()[0]
            ...
            return Response(
                {"success": True, "removed": deleted > 0, "action": "unrepost"}
            )
            ...
            if not chat_id:
                return Response(
                    {"error": "Требуется chat_id или target_id"}, status=400
                )
            ...
            message = Message.objects.create(
                sender=request.user, text=message_text, shared_post=original_post
            )
            ...
            if not group_id:
                return Response(
                    {"error": "Требуется group_id или target_id"}, status=400
                )
            ...
            try:
                ...
                logger.warning(f"Failed to copy media files for repost: {e}")
            ...
            if original_post.post_type == "system":
                return Response(
                    {"error": "Нельзя репостнуть системный пост"}, status=400
                )

            if original_post.visibility == "private":
                return Response(
                    {"error": "Нельзя репостнуть приватный пост"}, status=400
                )
            ...
            repost = Repost.objects.create(
                original_post=original_post, user=request.user, comment=comment
            )
        ...
        logger.error(f"Error in repost_post {post_id}: {e}", exc_info=True)
        return Response(
            {"error": "Ошибка при создании репоста", "detail": str(e)}, status=500
        )
...
def edit_post(request, post_id):
    ...
    try:
        ...
        if "spoiler_description" in request.data:
            post.spoiler_description = (
                request.data["spoiler_description"][:500]
                if request.data["spoiler_description"]
                else None
            )
        ...
        # Обработка множественных медиа файлов через _handle_media_uploads
        if any(key.startswith("media_") for key in request.FILES.keys()):
            # Импортируем метод из PostViewSet
            from .views import PostViewSet

            viewset = PostViewSet()
            viewset.request = request
            viewset._handle_media_uploads(request, post, replace=True)

        post.edited_at = timezone.now()
        post.save()

        # Обработка attachments (аниме, плейлисты, shorts)
        # Получаем новые ID из запроса
        anime_ids = request.data.get("anime_ids", [])
        playlist_ids = request.data.get("playlist_ids", [])
        reactor_ids = request.data.get("reactor_ids", [])

        # Удаляем старые attachments
        post.attachments.all().delete()

        # Создаём новые attachments для аниме
        if anime_ids:
        ...
        # Создаём новые attachments для плейлистов
        if playlist_ids:
        ...
        # Создаём новые attachments для shorts
        if reactor_ids:
        ...
        return Response(
            {"success": True, "message": "Пост обновлён", "post": serializer.data}
        )
    except Exception as e:
        logger.error(f"Error editing post {post_id}: {e}", exc_info=True)
        return Response(
            {"error": "Ошибка при редактировании поста", "detail": str(e)}, status=500
        )
...
def get_hidden_posts(request):
    ...
    hidden_posts = (
        UserPostHidden.objects.filter(user=request.user)
        .select_related("post")
        .order_by("-created_at")
    )
...
def get_user_posts(request, user_id):
    ...
    posts = Post.objects.filter(
        author=user, status="published", is_deleted=False
...
def get_group_posts(request, group_id):
    ...
    posts = Post.objects.filter(
        group=group, status="published", is_deleted=False
    ).order_by("-created_at")[:20]
...
def get_hashtag_posts(request, tag_name):
    ...
    posts = Post.objects.filter(
        hashtag_links__hashtag=hashtag, status="published", is_deleted=False
    ).order_by("-created_at")[:20]
...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def check(self, request):
        ...
        if not content_type_str or not object_id:
            return Response(
                {"error": "content_type и object_id обязательны"}, status=400
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"success": True, "is_favorited": True, "id": favorite.id}, status=201
        )
    ...
    def toggle(self, request):
        ...
        content_type_str = request.data.get("content_type") or request.query_params.get(
            "content_type"
        )
        object_id = request.data.get("object_id") or request.query_params.get(
            "object_id"
        )
...
def get_online_users(request):
    ...
    users = User.objects.filter(is_online=True).exclude(
...
class ChatInviteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        try:
            ...
            ).select_related("chat", "created_by")
        except Exception:
...
def mark_private_chat_read(request, chat_id):
    ...
    # DEBUG: Логируем ВСЁ что приходит
    logger.info(f"=== mark_private_chat_read START ===")
    ...
    # Получаем specific message_ids если переданы
    message_ids = request.data.get("message_ids", [])
    
    print(f"DEBUG mark_private_chat_read: chat_id={chat_id}, user={request.user.id}, message_ids={message_ids}")
    ...
    if message_ids:
        ...
        print(f"DEBUG: other_user={other_user.id if other_user else None}")
        
        # Фильтруем сообщения по IDs
        messages = Message.objects.filter(
            id__in=message_ids,
            private_chat=chat
        )

        # Создаём записи о прочтении и собираем IDs отправителей
        read_message_ids = []
        sender_ids = set()
        ...
        for message in messages:
            ...
            print(f"DEBUG: MessageReadStatus for msg {message.id}, user {request.user.id}: created={created}")
            read_message_ids.append(message.id)
            sender_ids.add(message.sender_id)

        print(f"DEBUG: read_message_ids={read_message_ids}, sender_ids={sender_ids}")
        
        # Отправляем WebSocket событие отправителям
        if read_message_ids and other_user:
            channel_layer = get_channel_layer()
            try:
                print(f"DEBUG: Sending WS event to chat_user_{other_user.id}")
                ...
                print(f"DEBUG: WS event sent successfully")
            except Exception as e:
                import logging
                logging.getLogger(__name__).error(f'Error sending read status: {e}')
                print(f"ERROR: {e}")
    else:
        # Отмечаем все сообщения как раньше
        chat.mark_as_read(request.user)
...
def mark_group_chat_read(request, chat_id):
    ...
    if message_ids:
        ...
        # Отправляем WebSocket событие всем отправителям (кроме себя)
        if read_message_ids:
            channel_layer = get_channel_layer()
            try:
...
def search_messages(request):
    ...
    messages = Message.objects.filter(text__icontains=query, chat_id=chat_id).order_by(
        "-created_at"
    )[:50]
...
class PostCommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            PostComment.objects.filter(post_id=post_id, is_deleted=False)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not content_id:
            content_id = (
                ...
                or request.data.get("post_id")
                or request.data.get("comment_id")
        ...
        if not content_id:
            ...
            return Response(
                {"error": "Поле content_id обязательно", "received_data": request.data},
                status=400,
            )
        ...
        try:
            ...
            return Response(
                {"error": f"Неверный формат content_id: {content_id}"}, status=400
            )
        ...
        if content_type == "post":
            ...
            if content_id == request.user.id:
                return Response(
                    {"error": "Нельзя пожаловаться на самого себя"}, status=400
                )
        ...
        logger.info(
            f"Report created: id={report.id}, type={content_type}, content_id={content_id}"
        )
        ...
        return Response(
            {"success": True, "id": report.id, "message": "Жалоба отправлена"},
            status=201,
        )
...
class SubscriptionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request):
        ...
        for i, user_data in enumerate(data):
            user_data["subscribed_at"] = (
                follows[i].created_at.isoformat() if follows else None
            )
...
class NotInterestedViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return UserNotInterested.objects.filter(user=self.request.user).select_related(
            "target_user"
        )
    ...
    def list(self, request):
        ...
        search = request.query_params.get("search", "")
        if search:
            not_interested = not_interested.filter(
        ...
        for i, user_data in enumerate(data):
            user_data["hidden_at"] = (
                not_interested[i].created_at.isoformat() if not_interested else None
            )
    ...
    def add_or_remove(self, request, user_id=None):
        ...
        if existing:
            existing.delete()
            return Response(
                {"hidden": False, "message": "Пользователь удалён из скрытых"}
            )
        else:
            reason = request.data.get("reason", "")
            UserNotInterested.objects.create(
                user=request.user, target_user=target_user, reason=reason
            )
            return Response(
                {"hidden": True, "message": "Пользователь добавлен в скрытые"}
            )
...
def forward_post_to_chat(request, chat_id=None):
    ...
    if chat_id is None:
        ...
        if not chat_id:
            return Response(
                {"error": "Требуется chat_id (в URL или в body)"}, status=400
            )
    ...
    if chat_type == "group":
        ...
        try:
            ...
            logging.getLogger(__name__).error(
                f"Error checking private chat membership: {e}"
            )
    ...
    message_text = request.data.get("message", "")
    message = Message.objects.create(
        sender=request.user, text=message_text, shared_post=post
    )
...
class ModerationReportViewSet(viewsets.ModelViewSet):
    ...
    def resolve(self, request, pk=None):
        ...
        action = request.data.get(
            "action"
        )  # 'delete_content', 'warn_author', 'ban_author', 'reject'
        comment = request.data.get("comment", "")
...
def get_extended_feed(request):
    ...
    # Базовый запрос
    queryset = Post.objects.filter(status="published", is_deleted=False)

    # Исключаем скрытые посты и профили
    hidden_post_ids = UserPostHidden.objects.filter(user=user).values_list(
        "post_id", flat=True
    )
    hidden_user_ids = UserNotInterested.objects.filter(user=user).values_list(
        "target_user_id", flat=True
    )
    queryset = queryset.exclude(id__in=hidden_post_ids).exclude(
        author_id__in=hidden_user_ids
    )
    ...
    if groups:
        group_ids = GroupMembership.objects.filter(user=user).values_list(
            "group_id", flat=True
        )
...
class NotInterestedViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        qs = UserNotInterested.objects.filter(user=request.user).select_related(
            "target_user"
        )
    ...
    def destroy(self, request, pk=None):
        ...
        if deleted:
            return Response(
                {"success": True, "message": "Пользователь убран из списка"}
            )
...
def hide_author_from_feed(request, user_id):
    ...
    if target_user == request.user:
        return Response({"error": "Нельзя скрыть самого себя"}, status=400)
    UserNotInterested.objects.get_or_create(user=request.user, target_user=target_user)
    return Response(
        {"success": True, "message": f"Посты от @{target_user.username} скрыты"}
    )
...
def get_franchise_discussions(request):
    ...
    franchise_groups = (
        GroupChat.objects.filter(discussion_type="franchise")
