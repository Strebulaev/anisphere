...
class FeedViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Post.objects.filter(
                status="published", is_deleted=False, visibility="public"
            )
            .select_related(
                "author",
                "anime",
                ...
                "playlist",
                "reactor_post",
                "playlist__cover_image",
            )
            .prefetch_related("media_files", "attachments")
            .annotate(playlist_items_count=Count("playlist__items"))
            .order_by("-created_at")
        )
    ...
    def weighted(self, request):
        ...
        try:
            ...
            all_posts = list(feed_data.get("pinned", [])) + list(
                feed_data.get("posts", [])
            )
            ...
            queryset = (
                Post.objects.filter(
                    id__in=post_ids, status="published", is_deleted=False
                )
                .select_related(
                    "author",
                    "anime",
    ...
    def hot(self, request):
        ...
        try:
            ...
            traceback.print_exc()
            return Response(
                {"error": str(e), "detail": "Error getting hot posts"}, status=500
            )
    ...
    def top(self, request):
        ...
        try:
            ...
            traceback.print_exc()
            return Response(
                {"error": str(e), "detail": "Error getting top posts"}, status=500
            )
    ...
    def trending(self, request):
        ...
        try:
            ...
            posts = (
                ...
                .annotate(
                    activity_score=F("likes_count")
                    + F("comments_count") * 2
                    + F("views_count"),
                    playlist_items_count=Count("playlist__items"),
                )
                .order_by("-activity_score", "-created_at")
                .select_related("author", "anime", "playlist", "group")
            ...
            traceback.print_exc()
            return Response(
                {"error": str(e), "detail": "Error getting trending posts"}, status=500
            )
    ...
    def followers(self, request):
        ...
        try:
            ...
            subscriptions = Follow.objects.filter(follower=user).values_list(
            ...
            traceback.print_exc()
            return Response(
                {"error": str(e), "detail": "Error getting followers feed"}, status=500
            )
