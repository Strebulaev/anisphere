  ...
  <div v-if="show" class="modal-overlay" @click="$emit('close')">
    <div class="modal danger-modal" @click.stop>
      ...
      <div class="modal-warning">
        <strong>Риски:</strong>
        <ul>
          <li>Аккаунт станет уязвимым к взлому</li>
          <li>Вам не потребуется код при входе</li>
          <li>Действие можно отменить, включив 2FA снова</li>
        </ul>
      </div>

      <div class="modal-actions">
        <button @click="$emit('close')" class="cancel-btn">Отмена</button>
        <button @click="$emit('confirm')" class="confirm-btn">Отключить</button>
      </div>
    </div>
  </div>
</template>
