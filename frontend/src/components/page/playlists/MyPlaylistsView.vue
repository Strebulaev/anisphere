...
const stats = computed(() => ({
  ...
  public: playlists.value.filter(p => p.is_public).length,
  private: playlists.value.filter(p => !p.is_public).length
}))
  ...
  <div class="my-playlists-page">
    ...
    <div class="tabs-container">
      <div class="tabs">
        ...
        >
          Публичные <span class="count">({{ stats.public }})</span>
        </button>
        <button 
          class="tab" 
        ...
        >
          Приватные <span class="count">({{ stats.private }})</span>
        </button>
      </div>
    </div>
    ...
    <div v-else-if="filteredPlaylists.length === 0" class="empty-state">
      <span class="icon">📝</span>
      <h3>{{ playlists.length === 0 ? 'У вас пока нет плейлистов' : 'Ничего не найдено' }}</h3>
      <p>{{ playlists.length === 0 ? 'Создайте свой первый плейлист и добавьте в него любимое аниме' : 'Попробуйте изменить параметры фильтрации' }}</p>
      <button v-if="playlists.length === 0" class="btn-create" @click="router.push('/playlists/create')">
        <span class="icon">+</span>
        Создать плейлист
      </button>
    </div>
