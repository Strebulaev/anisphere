  ...
  <div class="subscription-card">
    <div class="user-info" @click="goToProfile">
      ...
      <div class="user-details">
        ...
        <span class="followers">{{ formatCount(user.followers_count) }} подписчиков</span>
      </div>
    </div>

    <div class="card-actions">
      <button class="btn-message" @click="goToChat" title="Написать">
        💬
      </button>
      <button
        class="btn-unfollow"
        :class="{ loading: isUnfollowing }"
        @click="handleUnfollow"
        :disabled="isUnfollowing"
      >
        <span v-if="!isUnfollowing">✓ Подписан</span>
        <span v-else>...</span>
      </button>
    </div>
  </div>
