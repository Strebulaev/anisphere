        ...
        <div class="filter-section">
          <div class="checkbox-grid">
            <label class="checkbox-item">
              <input type="checkbox" v-model="localFilters.myPosts" @change="onAutoApply" />
              <span>Мои посты</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="localFilters.following" @change="onAutoApply" />
              <span>Подписки</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="localFilters.fromGroups" @change="onAutoApply" />
              <span>Из групп</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="localFilters.withAnime" @change="onAutoApply" />
              <span>С аниме</span>
            </label>
          </div>
        </div>
...
const props = defineProps<{ modelValue: FeedFilters }>()
...
const setPeriod = (v: FeedFilters['period']) => {
  localFilters.value.period = v
  applyFilters()
}

const setSort = (v: FeedFilters['sort']) => {
  localFilters.value.sort = v
  applyFilters()
}

const onAutoApply = () => {
  // применяем сразу при изменении чекбокса
  applyFilters()
}
...
.filters-header {
  ...
  padding: 0.75rem 1rem; cursor: pointer; user-select: none; transition: background 0.2s;
}
.filters-header:hover { background: #161616; }

.filters-title {
  color: #aaa; font-size: 0.9rem; display: flex; align-items: center; gap: 0.5rem;
}
.active-badge {
  background: #667eea; color: white; font-size: 0.7rem; width: 18px; height: 18px;
  border-radius: 50%; display: inline-flex; align-items: center; justify-content: center;
}
.toggle-icon { color: #555; font-size: 0.75rem; }

.filters-panel { border-top: 1px solid #1a1a1a; padding: 1rem; display: flex; flex-direction: column; gap: 1rem; }
.filter-section { display: flex; flex-direction: column; gap: 0.5rem; }
.section-label { color: #666; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; }

.checkbox-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; }
.checkbox-item { display: flex; align-items: center; gap: 0.5rem; color: #aaa; font-size: 0.875rem; cursor: pointer; }
.checkbox-item input[type="checkbox"] { accent-color: #667eea; width: 14px; height: 14px; }

.btn-group { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.period-btn, .sort-btn {
  background: #1a1a1a; border: 1px solid #2a2a2a; color: #888;
  padding: 0.375rem 0.75rem; border-radius: 20px; cursor: pointer;
  font-size: 0.8rem; transition: all 0.2s;
}
.period-btn.active, .sort-btn.active { background: #667eea; color: white; border-color: #667eea; }
.period-btn:hover:not(.active), .sort-btn:hover:not(.active) { background: #222; color: #aaa; }

.filter-actions { display: flex; justify-content: flex-end; gap: 0.5rem; }
.btn-reset { background: none; border: 1px solid #333; color: #666; padding: 0.5rem 1rem; border-radius: 8px; cursor: pointer; font-size: 0.85rem; }
.btn-apply { background: #667eea; border: none; color: white; padding: 0.5rem 1.25rem; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; }
.btn-apply:hover { background: #5a6fd6; }

.active-tags { display: flex; flex-wrap: wrap; gap: 0.5rem; padding: 0.5rem 1rem 0.75rem; border-top: 1px solid #1a1a1a; }
.filter-tag {
  background: rgba(102, 126, 234, 0.15); border: 1px solid rgba(102, 126, 234, 0.3);
  color: #8b9ef5; padding: 0.25rem 0.625rem; border-radius: 20px; font-size: 0.78rem; cursor: pointer; transition: all 0.2s;
}
.filter-tag:hover { background: rgba(102, 126, 234, 0.25); }
.clear-all { background: none; border: none; color: #555; font-size: 0.78rem; cursor: pointer; padding: 0.25rem 0.5rem; }
.clear-all:hover { color: #888; }
